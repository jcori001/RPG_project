...
'''
NOTES:
-enemies are hard coded to have x4 magnitude heal
-res is hard coded to work with revive only

Future Mechanics:
-defense and magic defense math to reduce dmg
-speed damage through magic
-elemental weakness
-add special attacks
'''
...
while running:
    for unit in players + enemies: 
        if unit.spd == unit.maxspd and unit.hp > 0:
            ...
            unit.manipulate_mp(int(round(unit.mp*0.03, 0)))
            #print status banner
            ...
            if unit in players:
                #run player loop
                player_turn = True
                print(bcolors.OKBLUE + f'\n\n{unit.name} charged {int(round(unit.mp*0.03, 0))} MP.' + bcolors.ENDC)
                ...
                if enemy_choice == 0:
                    ...
                    if magic.type == "heal":
                        print(f'\n{unit.name} casts {magic.name}...')
                        magic_dmg = magic.generate_damage()
                        unit.heal(magic_dmg*4) #set stronger heal for bosses
